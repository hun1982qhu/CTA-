...
class GridTradingStrategy(SpreadStrategyTemplate):
    ...
    def on_spread_bar(self, bar: BarData):
        ...
        if long_volume > 0:
            ...
            self.start_long_algo(long_price, long_volume, payup=5, interval=5)

        # 做空，检查最大持仓，和当前持仓的差值
        short_volume = self.max_target - self.spread_pos
        if short_volume < 0:
            short_price = bar.close_price - self.pay_up
            self.start_short_algo(short_price, abs(short_volume), payup=5, interval=5)

        if bar.close_price < self.grid_start - 10 * self.grid_price:
            self.start_short_algo(bar.close_price - 10, abs(self.spread_pos), payup=5, interval=5)
            self.trading = False
            self.put_event()
            self.write_log("亏太多了，不玩了")

        elif bar.close_price > self.grid_start + 10 * self.grid_price:
            self.start_long_algo(bar.close_price + 10, abs(self.spread_pos), payup=5, interval=5)
            self.trading = False
            self.put_event()
